...
from logic.posture_logic import analyze_posture, has_baseline, save_baseline
...
def save_baseline_pose():
    guide_msg = (
        "기준 자세를 저장합니다.\n"
        "바른 자세로 앉은 뒤 웹캠 창에서 q를 눌러주세요."
    )
    messagebox.showinfo("기준 자세 저장", guide_msg)
    ...
    try:
        ...
        if pose_data is None:
            messagebox.showwarning(
                "인식 실패",
                "기준 자세를 인식할 수 없습니다. 카메라 앞에 앉아 다시 시도해주세요.",
            )
            return

        baseline = save_baseline(pose_data)
        info_label.config(text="상태: 기준 자세 저장 완료")
        messagebox.showinfo(
            "기준 자세 저장 완료",
            "기준 자세가 저장되었습니다.\n이제 측정 시작 버튼을 눌러주세요.",
        )
...
def start_monitoring():
    ...
    if not has_baseline():
        messagebox.showwarning(
            "기준 자세 없음",
            "먼저 기준 자세 저장 버튼을 눌러 기준 자세를 저장해주세요.",
        )
        return

    guide_msg = (
        "정확한 측정을 위해 측정 중에는 어깨와 상체 위치를 고정하고, "
        "카메라와의 거리를 유지해주세요. 앞뒤로 움직이지 마세요."
    )
    ...
    try:
        ...
        info_label.config(text=f"상태: {result['message']}")
...
def stop_monitoring():
    ...
    try:
        ...
        messagebox.showinfo(
            ...
            f"경고 횟수: {warning_count}\n"
            "Firebase 저장 완료",
        )

        warning_count = 0
        last_result = None
        info_label.config(text="상태: 측정 대기 중")
